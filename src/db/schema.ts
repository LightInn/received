...
export const Chapter = pgTable("chapters", {
    ...
    storyId: uuid("story_id").references(() => Story.id).notNull(),
    title: varchar({length: 255}).notNull(),
    isDraft: boolean("is_draft").default(true).notNull(),
...
export const Conversation = pgTable("conversations", {
        id: uuid().primaryKey().defaultRandom().notNull(),
        storyId: uuid("story_id").references(() => Story.id).notNull(),
        chapterId: uuid("chapter_id").references(() => Chapter.id).notNull(),
        title: varchar({length: 255}).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
    }
...
export const Message = pgTable("messages", {
    ...
    // sender character
    senderId: uuid("sender_id").references(() => Character.id).notNull(),
...
export const PointOfView = pgTable("points_of_view", {
    id: uuid().primaryKey().defaultRandom().notNull(),
    chapterId: uuid("chapter_id").references(() => Chapter.id).notNull(),
    characterId: uuid("character_id").references(() => Character.id).notNull(),
});
