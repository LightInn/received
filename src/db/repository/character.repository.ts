...
"use server";
import { db } from '..';
import { CharacterSchema, StorySchema } from "../schema";
import { sql, eq } from "drizzle-orm";
import { Character } from "@/models/Character";

// Création d'un personnage
export async function createCharacter(data: Partial<Character>): Promise<string | null> {
    try {
...
export async function updateCharacter(characterId: string, data: Partial<Character>): Promise<boolean> {
    try {
        const result = await db
...
export async function deleteCharacter(characterId: string): Promise<boolean> {
    try {
        const result = await db.delete(CharacterSchema).where(eq(CharacterSchema.id, characterId));
        return result.rowCount == null ? false : result.rowCount > 0;
    } catch (error) {
