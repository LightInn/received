                                    ...
                                    <div key={chapter.id} className="flex items-center justify-between p-2 border-b">
                                        ...
                                        <div>
                                            <Button variant="ghost" size="sm"
                                                    onClick={() => setCurrentChapter(chapter.id)}>Edit</Button>
                                            <Button variant="ghost" size="sm" onClick={() => deleteChapter(chapter.id)}><Trash2
                            ...
                            <ScrollArea className="h-[300px]">
                                {characters.map(character => (
                                    <div key={character.id} className="flex items-center justify-between p-2 border-b">
                                        ...
                                        <Button variant="ghost" size="sm" onClick={() => deleteCharacter(character.id)}><Trash2
                                            className="h-4 w-4"/></Button>
                                    </div>
                                ))}
                            </ScrollArea>
                            ...
                            {currentChapter && (
                                <div className="mt-4 space-y-4">
                                    <Select onValueChange={setSelectedCharacter} value={selectedCharacter || undefined}>
                                        ...
                                        <SelectContent>
                                            {characters.map(character => (
                                                <SelectItem key={character.id}
                                                            value={character.id}>{character.name}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                        ...
                                        {chapters.find(chapter => chapter.id === currentChapter)?.conversations.map(message => {
                                            ...
                                            return (
                                                <div key={message.id} className="flex items-start space-x-2 mb-2">
                                                    ...
                                                    <Button variant="ghost" size="sm"
                                                            onClick={() => deleteMessage(currentChapter, message.id)}><Trash2
                                                        className="h-4 w-4"/></Button>
                                                </div>
                                            )
                                        })}
                                            ...
                                            {chapter.conversations.map(message => {
                                                ...
                                                return (
                                                    <div key={message.id} className="flex items-start space-x-2">
                                                        ...
                                                        <div
                                                            className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-2">
                                                            <div className="font-bold">{character?.name}</div>
                                                            {message.type === 'text' ? (
                                                                <p>{message.content}</p>
                                                            ) : (
