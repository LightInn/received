...
"use server";
import { db } from '..';
import { MessageSchema, ConversationSchema, CharacterSchema } from "../schema";
import { sql, eq } from "drizzle-orm";
import { Message } from "@/models/Message";

// Création d'un message
export async function createMessage(data: Partial<Message>): Promise<string | null> {
    try {
...
export async function deleteMessage(messageId: string): Promise<boolean> {
    try {
        const result = await db.delete(MessageSchema).where(eq(MessageSchema.id, messageId));
        return result.rowCount == null ? false : result.rowCount > 0;
    } catch (error) {
